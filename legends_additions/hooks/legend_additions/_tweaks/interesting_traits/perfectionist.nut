::mods_hookExactClass("skills/traits/natural_trait", function (o)
{
    o.create = function()
	{
		...
		this.m.Description = "Is a perfectionist, removing any margin of error in combat.";
		this.m.Titles = [
			"the Talented",
			"the Natural",
			"the Gifted"
    ...
    o.getTooltip = function()
	{
		return [
			...
			},
			{
				...
				type = "text",
				icon = "ui/icons/special.png",
				text = "Enemy's hit dice against this character have a lower bound of 6."
			}
		];
	}
...
::mods_hookBaseClass("skills/skill", function(o) {
    ...
    o.attackEntity = function( _user, _targetEntity, _allowDiversion = true )
	{
		...
		if (isHit && this.Math.rand(1, 100) <= _targetEntity.getCurrentProperties().RerollDefenseChance)
		{
			...
			if (!isHit)
			{
				this.Tactical.EventLog.logEx(::Const.UI.getColorizedEntityName(_targetEntity) + " got lucky.");
			}
			else
			{
				this.Tactical.EventLog.logEx(::Const.UI.getColorizedEntityName(_targetEntity) + " wasn\'t lucky enough.");
			}
		}
