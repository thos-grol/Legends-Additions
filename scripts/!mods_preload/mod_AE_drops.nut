this.getroottable().AE.drop_potion <- function (_killer, _skill, _tile, _fatalityType, chance, item)
{
	if (_tile != null && _killer == null || _killer.getFaction() == this.Const.Faction.Player || _killer.getFaction() == this.Const.Faction.PlayerAnimals)
	{
		...
		{
			if (i >= 25) break;
			if (bro.getSkills().hasSkill("trait.bloodline_researcher")) count++;
		}
		if (this.World.Assets.getOrigin().getID() == "scenario.anatomists") count *= 2.0;
...
this.getroottable().AE.hook_drops <- function ()
{
	::mods_hookExactClass("entity/tactical/enemies/alp", function (o)
	{
		...
		o.onDeath = function(_killer, _skill, _tile, _fatalityType)
		{
			...
			this.getroottable().AE.drop_potion(_killer, _skill, _tile, _fatalityType, chance, item);
		}
	});
