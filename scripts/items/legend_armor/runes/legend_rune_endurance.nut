this.legend_rune_endurance <- this.inherit("scripts/items/legend_armor/legend_armor_upgrade", {
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "This item has the power of the rune sigil of Endurance:\n[color=" + ::Const.UI.Color.PositiveValue + "]+" + this.m.RuneBonus1 + "[/color] Fatigue recovery per turn.\n[color=" + ::Const.UI.Color.PositiveValue + "]-" + this.m.RuneBonus2 + "%[/color] Fatigue cost and effects multiplier."
		});
		return result;
	}
