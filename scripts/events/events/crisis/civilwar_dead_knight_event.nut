this.civilwar_dead_knight_event <- this.inherit("scripts/events/event", {
	...
	function onUpdateScore()
	{
		...
		{
			if (bro.getBackground().getID() == "background.thief" || bro.getBackground().getID() == "background.female_thief" || bro.getBackground().getID() == "background.legend_commander_assassin" || bro.getSkills().hasSkill("trait.eagle_eyes"))
			{
				candidates.push(bro);
			}
		}
