this.bastard_assassin_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "C",
			Text = "[img]gfx/ui/events/event_33.png[/img]Before you decide anything, you ask the assassin how much was offered to slay the bastard. He weighs the numbers by tilting his head side to side.%SPEECH_ON%Well, it was... and then, deducting travel time, equipment costs, the time it took to find the damn sod, and the time it took scouting your encampment and figuring out whether you\'d be open to dialogue I\'d say... five-hundred crowns on your bill. You still up for that sort of \'discussion\', hm?%SPEECH_OFF%",
			Image = "",
			List = [],
			Characters = [],
			Options = [
				...
				{
					Text = "I will pay you those 500 crowns, and both %bastard% and you will stay.",
					function getResult( _event )
					{
						return "D";
					}
	...
	function onUpdateScore()
	{
		...
		foreach( bro in brothers )
		{
			if (bro.getLevel() >= 6 && bro.getBackground().getID() == "background.bastard") candidates.push(bro);
			else if (!bro.getSkills().hasSkill("trait.player")) candidates_other.push(bro);
