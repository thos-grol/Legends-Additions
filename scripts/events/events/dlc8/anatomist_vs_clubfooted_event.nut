this.anatomist_vs_clubfooted_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "B",
			Text = "[img]gfx/ui/events/event_05.png[/img]{You give the anatomist the go-ahead. Him and the clubfooted man walk-and-hobble off to handle the business. Naturally, you go to have a look. You watch as the anatomist brings %clubfoot%\'s foot up on a stool. He takes a piece of wood out that already has teeth marks augured into it. He then takes out a vial of liquid, pops the cork, lathers the wood in it, takes a drink of it himself, and then hands the rest to his patient. %clubfoot% chugs the drink, then bites down on the wood. What follows is a disgusting series of leg breaks and castings and re-castings. The anatomist starts cutting in with a scalpel, grinning madly as he works. %clubfoot% has long since passed out.\n\nWhen it\'s all said and done, %clubfoot%\'s leg is totally wrecked and casted up. The anatomist says the operation was a success, though there will be a somewhat lengthy recovery time that will be necessary. The foot will have to be re-casted again and again, and each time moving the foot a little more, but it can be done. A delirious %clubfoot% is smiling as he looks down at his foot.%SPEECH_ON%It will be worth it, captain. For me, and also for the %companyname%.%SPEECH_OFF%The rather dutiful and drugged up mercenary then falls backwards and into a snoring sleep.}",
			Image = "",
			List = [],
			Characters = [],
			Options = [
	...
	function onUpdateScore()
	{
		...
		{
			if (bro.getSkills().hasSkill("trait.clubfooted")) clubfootedCandidates.push(bro);
		}
