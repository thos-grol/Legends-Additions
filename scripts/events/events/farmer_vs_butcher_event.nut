this.farmer_vs_butcher_event <- this.inherit("scripts/events/event", {
	...
	function onUpdateScore()
	{
		...
		{
			if (bro.getLevel() <= 6 && bro.getBackground().getID() == "background.butcher" || bro.getLevel() <= 6 && bro.getBackground().getID() == "background.female_butcher")
			{
		...
		foreach( bro in brothers )
		{
			if (bro.getLevel() <= 6 && bro.getBackground().getID() == "background.farmhand" || bro.getLevel() <= 6 && bro.getBackground().getID() == "background.female_farmhand")
			{
