this.more_action_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			...
			Options = [
				{
					Text = "Good to have the company burn for battle like this.",
					function getResult( _event )
					{
						return 0;
					}
		...
		this.m.Screens.push({
			ID = "C",
			Text = "[img]gfx/ui/events/event_64.png[/img]You try and explain to the mercenaries that, regardless of whether or not they\'re fighting, they are going to be getting paid. But money is not their primary concern. They truly wish to fight and you words have little effect on their rather earnest attitudes.",
			Image = "",
			List = [],
			Characters = [],
			Options = [
