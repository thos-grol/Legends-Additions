this.bird_shits_on_sellsword_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "A",
			Text = "%terrainImage%{While traveling the land, %birdbro% gets struck by birdshite. It hits their swordhand and splashes crosswise all over their armor.%SPEECH_ON%Aww, awwww!%SPEECH_OFF%Their arms go wide like chicken wings as he looks at the damage.%SPEECH_ON%Bloody hell, just my luck!%SPEECH_OFF%}",
			Image = "",
			List = [],
			Characters = [],
			Options = [
