this.oldguard_becomes_drunkard_event <- this.inherit("scripts/events/event", {
	...
	function onUpdateScore()
	{
		...
		foreach( bro in brothers )
		{
			...
			if (bro.getLevel() >= 7 && !bro.getSkills().hasSkill("trait.drunkard") && this.World.getTime().Days - bro.getDaysWithCompany() < fallen[0].Time && this.World.getTime().Days - bro.getDaysWithCompany() < fallen[1].Time && !bro.getSkills().hasSkill("trait.player") && !bro.getFlags().get("IsPlayerCharacter"))
			{
				candidates.push(bro);
			}
		}
