this.defend_settlement_cultists_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Reward = 0,
		Kidnapper = null,
		Militia = null
	...
	function create()
	{
		...
		this.m.Type = "contract.defend_settlement_cultists";
		this.m.Name = "Defend Settlement";
		this.m.Description = "Scouts report strange men watching from afar. Folks are uneasy.";
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_20.png[/img]{A few peasants are roaming outside the halls of the room. You can hear their shouting and it is of a nervous tone. %employer% pours a drink and sips it with a shaking hand.%SPEECH_ON%I\'ll just be clear with you, sellsword. We have many, many reports of strange men just eerily watching from afar... Clearly, we\'ve no reason to doubt the seriousness of these reports. So, the question is, will you protect us?%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
