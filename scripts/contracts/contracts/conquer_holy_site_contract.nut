this.conquer_holy_site_contract <- this.inherit("scripts/contracts/contract", {
	...
	function start()
	{
		...
		this.m.Flags.set("Commander", ::Const.Strings.CharacterNames[::Math.rand(0, ::Const.Strings.CharacterNames.len() - 1)]);
	...
	function spawnAlly()
	{
		...
		local party = f.spawnEntity(tiles[0].Tile, candidates[::Math.rand(0, candidates.len() - 1)].getNameOnly() + " Company", true, ::Const.World.Spawn.Noble, 170 * this.getDifficultyMult() * this.getScaledDifficultyMult());
	...
	function spawnEnemy()
	{
		...
		local party = f.spawnEntity(tiles[0].Tile, "Regiment of " + candidates[::Math.rand(0, candidates.len() - 1)].getNameOnly(), true, ::Const.World.Spawn.Southern, ::Math.rand(100, 140) * this.getDifficultyMult() * this.getScaledDifficultyMult());
		party.getSprite("body").setBrush(party.getSprite("body").getBrush().Name + "_" + f.getBannerString());
