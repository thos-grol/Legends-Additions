this.escort_caravan_contract <- this.inherit("scripts/contracts/contract", {
	...
	function onTimedOut()
	{
		//TODO: change caravan spawns, full guard for npcs and half guard for contract.  Based on their strength, players they take main role or supportive role with different pay.
		//TODO: overhaul caravan loot.
		...
		if (this.m.Home.getProduce().len() != 0)
		{
			...
			if (::Legends.Mod.ModSettings.getSetting("WorldEconomy").getValue())
			{
				produce = this.Math.max(3, 3 + this.Math.round(0.025 * this.m.Home.getResources()));
			}
		...
		if (::Legends.Mod.ModSettings.getSetting("WorldEconomy").getValue())
		{
			local resources = this.Math.max(1, this.Math.round(0.025 * this.m.Home.getResources()));
			this.m.Home.setResources(this.m.Home.getResources() - resources);
			party.setResources(resources);
			local r = this.Math.rand(1, 3);
