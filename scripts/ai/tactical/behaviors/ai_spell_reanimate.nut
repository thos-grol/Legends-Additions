this.ai_spell_reanimate <- this.inherit("scripts/ai/tactical/behavior", {
	...
	function onEvaluate( _entity )
	{
		...
		foreach( t in potentialDanger )
		{
			...
			if (d.Turns <= 1.0) currentDanger = currentDanger + (1.0 - d.Turns);
		}
		...
		foreach( c in corpses )
		{
			...
			if (!canResurrectOnTile(c)) continue;
			if (this.getAgent().getIntentions().IsDefendingPosition && !this.m.Skill.isInRange(c)) continue;

			local score = 1.0;
			local dist = c.getDistanceTo(myTile);

			if (dist > this.Const.AI.Behavior.RaiseUndeadMaxDistance) continue;
			if (this.m.Skill.isInRange(c) && !this.m.Skill.onVerifyTarget(myTile, c)) continue;
			...
			score = score + c.Properties.get("Corpse").Value * this.Const.AI.Behavior.RaiseUndeadStrengthMult;
		...
		foreach( t in potentialCorpses )
		{
			...
			else
			{
				...
				while (currentDanger >= this.Const.AI.Behavior.RaiseUndeadMaxDanger);
	...
	function onExecute( _entity )
	{
		...
		if (this.m.IsFirstExecuted)
		{
			if (this.m.IsTravelling)
			{
				...
				{
					this.logInfo("* " + _entity.getName() + ": Moving into range to use Reanimate");
				}
