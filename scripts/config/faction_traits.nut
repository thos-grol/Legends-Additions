...
gt.Const.FactionTrait <- {
	...
	Actions = [
		...
		[],
		[
			//NobleHouse, Citadels and forts
			"scripts/factions/actions/defend_military_action", //spawns guards
			"scripts/factions/actions/move_troops_action",//spawns patrols - patrols based on faction strength
			"scripts/factions/actions/patrol_roads_action",
			"scripts/factions/actions/add_random_situation_action", //merge into update action
			// "scripts/factions/actions/send_ship_action", //WTF send and recieve have the same code??
			// "scripts/factions/actions/receive_ship_action",
			//"scripts/factions/actions/burn_location_action", //Merge into other actions
		...
		[
			//Settlement
			"scripts/factions/actions/update_settlement_action",
		...
		[
			...
			//add action to put situation on random town in each zone with check. if yes, beast infestation
			//if contract expire, has chance to be completed by other mercs. if fail, then increase beast faction strength
			//beast faction has 3 strengths for north, mid, and low
			//this.World.Statistics.getFlags() to store data
		],
		[
			//Barbarians
