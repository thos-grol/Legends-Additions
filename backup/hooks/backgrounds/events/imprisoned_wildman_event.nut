::mods_hookExactClass("events/events/imprisoned_wildman_event", function (o)
{
	o.create = function()
	{
		...
		this.m.Screens.push({
			ID = "AnimalsFreed",
			Text = "[img]gfx/ui/events/event_27.png[/img]You feel it just wouldn\'t be right to leave them out here to starve to death and a caged creature no doubt has gamey meat when it comes to harvesting. You decide to let them on out of their cages. Most of these strange creatures make a beeline for the treeline, but a hooded falcon remains behind, seemingly looking for a master.",
			Image = "",
			List = [],
			Characters = [],
			Options = [
