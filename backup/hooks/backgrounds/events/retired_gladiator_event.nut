::mods_hookExactClass("events/events/dlc6/retired_gladiator_event", function (o)
{
	o.create = function()
	{
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				local a = ::new("scripts/items/armor/oriental/gladiator_harness");
				local u = ::new("scripts/items/armor_upgrades/heavy_gladiator_upgrade");
				a.setUpgrade(u);
				this.List.push({
					id = 12,
				...
				this.List.push({
					...
					text = "You lose [color=" + ::Const.UI.Color.NegativeEventValue + "]1,000[/color] Crowns"
				});
			}
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				local trait = ::new("scripts/skills/traits/old_trait");
